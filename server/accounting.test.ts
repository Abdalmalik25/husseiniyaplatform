...
const llmAvailable = () => !!process.env.LLM_API_URL && !!process.env.LLM_API_KEY;
...
describe("Al-Husainia Accounting System Routers", () => {
  it.skipIf(!dbAvailable())("retrieves settings successfully", async () => {
  ...
  it.skipIf(!dbAvailable())("retrieves chart of accounts successfully", async () => {
    const ctx = createTestContext();
    const caller = appRouter.createCaller(ctx);
  ...
  it.skipIf(!dbAvailable() || !llmAvailable())("retrieves smart suggestions and insights", async () => {
    const ctx = createTestContext();
    const caller = appRouter.createCaller(ctx);
    const suggestions = await caller.accounting.getSmartSuggestions({ operationType: "إيراد" });

    expect(suggestions).toBeDefined();
    expect(suggestions.insights.length).toBeGreaterThan(0);
  }, 15000);

  it.skipIf(!dbAvailable())("adds and retrieves financial transactions successfully", async () => {
    const ctx = createTestContext();
    const caller = appRouter.createCaller(ctx);
    const accounts = await caller.accounting.getAccounts();
