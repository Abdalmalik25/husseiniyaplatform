    ...
    <div className="min-h-screen bg-background" dir="rtl">
      ...
      <main className="max-w-6xl mx-auto px-4 py-6 space-y-6">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h1 className="text-2xl font-black">القوائم المالية — تقارير الخادم</h1>
            <p className="text-xs text-muted-foreground">
              حسابات موثوقة من قاعدة البيانات مباشرة وفق معايير القيد المزدوج — مصدر حقيقة واحد
            </p>
          </div>
          <div className="flex items-end gap-2">
            ...
            {asOf && (
              <Button variant="ghost" className="h-9 text-xs" onClick={() => setAsOf("")}>
                مسح
              </Button>
            )}
          </div>
        ...
        <Tabs defaultValue="trial" className="space-y-4">
                  ...
                  <TableBody>
                    {trial.data?.rows?.map((r: any) => (
                      <TableRow key={r.accountId}>
                        ...
                        <TableCell className="font-mono">{r.debit ? fmt(r.debit) : "—"}</TableCell>
                        <TableCell className="font-mono">{r.credit ? fmt(r.credit) : "—"}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          <TabsContent value="income">
                    ...
                    <TableBody>
                      {income.data?.revenues?.map((r: any) => (
                        <TableRow key={r.code}>
                          ...
                          <TableCell className="font-mono">{fmt(r.amount)}</TableCell>
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <SummaryStat label="الإيرادات" value={fmt(income.data?.totals.revenue)} />
              <SummaryStat label="المصروفات" value={fmt(income.data?.totals.expense)} />
              <SummaryStat label="صافي الدخل" value={fmt(income.data?.totals.net)} />
            </div>
          </TabsContent>
          ...
          <TabsContent value="sheet">
            <div className="grid md:grid-cols-3 gap-4">
              {[
                { title: "الأصول", icon: <Wallet className="w-4 h-4" />, rows: sheet.data?.assets },
                { title: "الخصوم", icon: <Landmark className="w-4 h-4" />, rows: sheet.data?.liabilities },
                { title: "رأس المال", icon: <Users className="w-4 h-4" />, rows: sheet.data?.equity },
              ].map(s => (
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <SummaryStat label="الأصول" value={fmt(sheet.data?.totals.assets)} />
              <SummaryStat label="الخصوم" value={fmt(sheet.data?.totals.liabilities)} />
              <SummaryStat label="رأس المال" value={fmt(sheet.data?.totals.equity)} />
            </div>
          </TabsContent>

          <TabsContent value="cash">
            <ReportCard title="التدفقات النقدية" desc="صافي التغير في الحسابات النقدية">
              {cash.isLoading ? <Loading /> : (
                <Table>
                  <Header cols={["الكود", "الحساب", "صافي التدفق"]} numberOnly/>
                  <TableBody>
                    {cash.data?.lines?.map((r: any) => (
                      <TableRow key={r.accountId}>
                        <TableCell className="font-mono">{r.code}</TableCell>
                        <TableCell>{r.name}</TableCell>
                        <TableCell className="font-mono">{fmt(r.net)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
          ...
          {["ar", "ap"].map(k => {
            ...
            return (
              <TabsContent key={k} value={k}>
                <ReportCard
                  title={isAr ? "ذمم العملاء المدينة (Aging)" : "ذمم الموردين الدائنة (Aging)"}
                  icon={isAr ? <Users className="w-4 h-4" /> : <Factory className="w-4 h-4" />}
                  footer={`إجمالي: ${fmt(Object.values(data?.totals ?? {}).reduce((a: number, b: any) => a + (Number(b) || 0), 0))}`}
                >
                  {(isAr ? ar.isLoading : ap.isLoading) ? <Loading /> : (
                    <Table>
                      <Header cols={["الفاتورة", "الطرف", "الاستحقاق", "الفئة", "المتبقي"]} />
                      <TableBody>
                        {data?.rows?.map((r: any, i: number) => (
                          <TableRow key={i}>
                            ...
                            <TableCell>
                              <Badge
                                variant={r.bucket === "current" ? "outline" : "destructive"}
                                className="text-[10px]"
                              >
                                {STATUS_LABEL[r.bucket] ?? r.bucket}
                              </Badge>
                            </TableCell>
                            <TableCell className="font-mono">{fmt(r.outstanding)}</TableCell>
                          </TableRow>
                        ))}
                      </TableBody>
                    </Table>
