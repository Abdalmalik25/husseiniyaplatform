...
import { Package, Calendar, AlertTriangle, Plus, Search, Filter, Trash2, Eye, Edit, Lock, Unlock, CheckCircle } from "lucide-react";
...
export function StockReservationsPanel() {
  ...
  const { data: reservations, isLoading, refetch } = trpc.products.reservationList.useQuery(
    {
  ...
  const totalReservations = useMemo(() => reservations?.length || 0, [reservations]);
  const activeCount = useMemo(() => reservations?.filter(r => r.status === "active").length || 0, [reservations]);
  const totalQty = useMemo(() => reservations?.reduce((s, r) => s + (r.quantity || 0), 0) || 0, [reservations]);
  const fulfilledCount = useMemo(() => reservations?.filter(r => r.status === "fulfilled").length || 0, [reservations]);
  ...
  const handleCreateReservation = (e: React.FormEvent) => {
    e.preventDefault();
    if (!reservationForm.productId || !reservationForm.warehouseId || !reservationForm.quantity) {
    ...
    createReservation.mutate({
      ...
      batchId: reservationForm.batchId ? Number(reservationForm.batchId) : undefined,
      quantity: Number(reservationForm.quantity),
      source: reservationForm.source as any,
      sourceId: reservationForm.sourceId ? Number(reservationForm.sourceId) : undefined,
      sourceType: reservationForm.sourceType || undefined,
      customerId: reservationForm.customerId ? Number(reservationForm.customerId) : undefined,
    ...
    <div className="space-y-4">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">إدارة الحجوزات والتخصيصات لأوامر المبيعات والإنتاج</p>
        </div>
        ...
        <>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
            <Card className="border-0 shadow-sm bg-white p-3">
              ...
              <p className="font-bold text-lg text-[#102a2b]">{totalReservations}</p>
            </Card>
            <Card className="border-0 shadow-sm bg-white p-3">
            ...
            <Card className="border-0 shadow-sm bg-white p-3">
              ...
              <p className="font-bold text-lg text-green-600">{fulfilledCount}</p>
            </Card>
          </div>
                ...
                <table className="w-full text-xs">
                  ...
                  <tbody>
                    {filteredReservations.map(res => {
                      const isExpired = res.expiresAt && new Date(res.expiresAt) < new Date();
                      const isActive = res.status === "active";
                      return (
                        <tr key={res.id} className={`border-b hover:bg-gray-50 ${isExpired && isActive ? "bg-red-50" : ""}`}>
                          <td className="p-2">
                            <Badge variant="outline" className="text-[9px]">
                              {sourceLabels[res.source] || res.source}
                            </Badge>
                          </td>
                          <td className="p-2 font-mono text-[10px]">{res.sourceId || "-"}</td>
                          <td className="p-2">
                            <div className="font-medium text-[11px]">{res.productName}</div>
                            <div className="text-[9px] text-gray-400">{res.productCode}</div>
                          </td>
                          <td className="p-2 text-[10px]">{res.batchNumber || "-"}</td>
                          <td className="p-2 text-center font-mono">{formatNum(res.quantity || 0)}</td>
                          <td className="p-2 text-center">
                            <Badge className={statusColors[res.status] || "bg-gray-100 text-gray-700"} variant="outline">
                              {statusLabels[res.status] || res.status}
                            </Badge>
                          </td>
                          <td className="p-2 text-center text-[10px]">{res.customerId ? `عميل #${res.customerId}` : "-"}</td>
                          <td className="p-2 text-center text-[10px]">
                            {res.expiresAt ? (
                              <span className={isExpired ? "text-red-600 font-bold" : "text-gray-600"}>
                                {new Date(res.expiresAt).toLocaleDateString("ar-EG")}
                              </span>
                            ) : "-"}
                          </td>
                            ...
                            {isActive && (
                              <>
                                <Button
                                  ...
                                  onClick={() => fulfillReservation.mutate({ id: res.id })}
                                ...
                                <Button
                                  ...
                                  onClick={() => releaseReservation.mutate({ id: res.id, reason: "تحرير يدوي" })}
                    ...
                    {filteredReservations.length === 0 && (
                      <tr><td colSpan={9} className="text-center text-gray-400 py-8">لا توجد حجوزات</td></tr>
                    )}
                  </tbody>
                </table>
              )}
          ...
          <form onSubmit={handleCreateReservation} className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <Select value={reservationForm.productId} onValueChange={v => setReservationForm({...reservationForm, productId: v})}>
              ...
              <div>
                ...
                <Select value={reservationForm.warehouseId} onValueChange={v => setReservationForm({...reservationForm, warehouseId: v})}>
                  <SelectTrigger className="h-9 text-xs">
            ...
            <div>
              ...
              <Select value={reservationForm.source} onValueChange={v => setReservationForm({...reservationForm, source: v})}>
                <SelectTrigger className="h-9 text-xs">
            ...
            <div className="grid grid-cols-3 gap-3">
              <div>
                ...
                <Input type="number" className="h-9 text-xs" value={reservationForm.sourceId} onChange={e => setReservationForm({...reservationForm, sourceId: e.target.value})} placeholder="اختياري" />
              </div>
              <div>
                <Label className="text-[11px]">الكمية *</Label>
                <Input type="number" min="1" className="h-9 text-xs" value={reservationForm.quantity} onChange={e => setReservationForm({...reservationForm, quantity: Number(e.target.value) || 1})} />
              </div>
              <div>
                <Label className="text-[11px]">الدفعة</Label>
                <Input type="number" className="h-9 text-xs" value={reservationForm.batchId} onChange={e => setReservationForm({...reservationForm, batchId: e.target.value})} placeholder="اختياري" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <Label className="text-[11px]">العميل</Label>
                <Select value={reservationForm.customerId} onValueChange={v => setReservationForm({...reservationForm, customerId: v})}>
                  <SelectTrigger className="h-9 text-xs">
              ...
              <div>
                ...
                <Input type="date" className="h-9 text-xs" value={reservationForm.expiresAt} onChange={e => setReservationForm({...reservationForm, expiresAt: e.target.value})} />
              </div>
            </div>
            <div>
              <Label className="text-[11px]">ملاحظات</Label>
              <Input className="h-9 text-xs" value={reservationForm.notes} onChange={e => setReservationForm({...reservationForm, notes: e.target.value})} placeholder="ملاحظات إضافية" />
            </div>
            <DialogFooter className="flex justify-end gap-2">
              <Button type="button" variant="outline" size="sm" onClick={() => setShowCreateDialog(false)}>إلغاء</Button>
              <Button type="submit" size="sm" className="bg-[#b87945] hover:bg-[#a06838] text-[#102a2b]" disabled={createReservation.isPending}>
                {createReservation.isPending ? "جاري الإنشاء..." : "إنشاء الحجز"}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
