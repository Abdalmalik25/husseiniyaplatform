...
export default function Portal() {
  ...
  const handleSearch = (e: React.FormEvent) => {
    e.preventDefault();
    if (!searchQuery.trim()) {
      toast.error("يرجى إدخال رقم الهاتف أو رقم الطلب المرجعي");
      return;
    }
    setSearched(true);
    toast.success("تم جلب نتائج البحث بنجاح");
  };
  ...
  const handlePrintPdf = (order: (typeof orders)[0]) => {
    openPrintableInvoiceWindow({
      ...
      paidAmount: 0,
      notes: order.deliveryNotes ?? `حالة الطلب: ${statusMap[order.status]?.text ?? order.status} — المسؤول: ${order.assignedTo ?? "غير محدد"}`,
    });
  };
  ...
  return (
    <div
      ...
      dir="rtl"
    >
      <HeaderNavbar institutionName="بوابة تتبع المشاريع والطلبات" />
                  ...
                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <Badge
                          className={statusMap[order.status]?.color ?? "bg-amber-600"}
                        >
                          {statusMap[order.status]?.text ?? order.status}
                        </Badge>
                      </div>
                      <CardTitle className="text-base font-bold text-slate-900 mt-2 font-display">
                        {order.deliveryAddress ?? `طلب رقم ${order.orderNumber}`}
                      </CardTitle>
                    </div>

                    <div className="text-left font-mono text-xs text-slate-500">
                      <div>التاريخ: {order.createdAt ? new Date(order.createdAt).toLocaleDateString("ar-EG") : "—"}</div>
                ...
                <CardContent className="p-4 sm:p-5 space-y-4">
                  ...
                  {order.deliveryNotes && (
                    <div className="bg-slate-50 p-3.5 rounded-xl border border-slate-200 space-y-2 text-xs">
                      ...
                      <p className="text-slate-600 pr-2">{order.deliveryNotes}</p>
                    </div>
                  )}
