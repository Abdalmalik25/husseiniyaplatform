    ...
    <div className="container flex flex-col gap-6 py-8 lg:flex-row">
      ...
      <main className="min-w-0 flex-1">
        ...
        {(activeTab === "sales" || activeTab === "purchases") && <div className="space-y-4">
          ...
          <Card className="border-0 shadow-soft"><CardHeader><CardTitle className="font-display">الفواتير ({invoices.data?.length ?? 0})</CardTitle></CardHeader><CardContent>{invoices.data?.length ? <div className="space-y-2">{invoices.data.map((inv) => <div key={inv.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4"><div><b className="text-sm">{inv.invoiceNumber}</b><p className="text-xs text-slate-500">{inv.type === "sales" ? "مبيعات" : "مشتريات"} · {new Date(inv.createdAt).toLocaleDateString("ar")}</p></div><div className="flex items-center gap-3"><span className="font-bold">{Number(inv.total).toLocaleString("ar")}</span><Badge variant={inv.status === "paid" ? "default" : "outline"}>{inv.status === "paid" ? "مدفوعة" : inv.status === "issued" ? "صادرة" : inv.status}</Badge></div></div>)}</div> : <Empty text="لا توجد فواتير بعد." />}</CardContent></Card>
        </div>}
        ...
        {activeTab === "orders" && <div className="space-y-4">
          ...
          <Card className="border-0 shadow-soft"><CardHeader><CardTitle className="font-display">الطلبات ({orders.data?.length ?? 0})</CardTitle></CardHeader><CardContent>{orders.data?.length ? <div className="space-y-2">{orders.data.map((o) => <div key={o.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4"><div><b className="text-sm">{o.orderNumber}</b><p className="text-xs text-slate-500">{new Date(o.createdAt).toLocaleDateString("ar")}</p></div><div className="flex items-center gap-3"><span className="font-bold">{Number(o.total).toLocaleString("ar")}</span><Badge variant={o.status === "delivered" ? "default" : "outline"}>{o.status}</Badge></div></div>)}</div> : <Empty text="لا توجد طلبات بعد." />}</CardContent></Card>
        </div>}
        ...
        {activeTab === "payments" && <div className="space-y-4">
          ...
          <Card className="border-0 shadow-soft"><CardHeader><CardTitle className="font-display">المدفوعات ({payments.data?.length ?? 0})</CardTitle></CardHeader><CardContent>{payments.data?.length ? <div className="space-y-2">{payments.data.map((p) => <div key={p.id} className="flex items-center justify-between rounded-2xl bg-slate-50 p-4"><div><b>{p.type === "receive" ? "قبض" : "دفع"}</b><p className="text-xs text-slate-500">{p.method} · {new Date(p.createdAt).toLocaleDateString("ar")}</p></div><b className={p.type === "receive" ? "text-emerald-700" : "text-copper"}>{Number(p.amount).toLocaleString("ar")}</b></div>)}</div> : <Empty text="لا توجد مدفوعات بعد." />}</CardContent></Card>
        </div>}
