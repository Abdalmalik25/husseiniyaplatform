...
export type UseDataGridKeyboardOptions = {
  rows: DataGridRow[];
  columns: { key: string; header: string; editable?: boolean; editorType?: DataGridCell["editorType"]; options?: DataGridCell["options"] }[];
  selection: DataGridSelection;
  onSelectionChange: (selection: DataGridSelection) => void;
  onCellEdit?: (rowIndex: number, columnKey: string, value: any) => void;
  onRowAction?: (action: "duplicate" | "delete" | "insert-above" | "insert-below", rowIndex: number) => void;
...
export function useDataGridKeyboard(options: UseDataGridKeyboardOptions) {
  ...
  const handleIntent = useCallback((detail: any) => {
    ...
    const getNextEditableColumn = (currentKey: string, direction: "next" | "prev") => {
      const idx = editableColumns.indexOf(currentKey);
      if (idx === -1) return editableColumns[0] ?? null;
      const nextIdx = direction === "next" ? idx + 1 : idx - 1;
      if (nextIdx >= 0 && nextIdx < editableColumns.length) return editableColumns[nextIdx];
      return direction === "next" ? editableColumns[0] : editableColumns[editableColumns.length - 1];
    };
    ...
    switch (detail.intent) {
      ...
      case "tab":
        if (isEditing) {
          ...
          const nextEditable = getNextEditableColumn(currentColumnKey, "next");
          if (nextEditable) {
      ...
      case "shift-tab":
        if (isEditing) {
          detail.preventDefault();
          detail.stopPropagation();
          const prevEditable = getNextEditableColumn(currentColumnKey, "prev");
          if (prevEditable) {
      ...
      case "arrow-left":
        ...
        if (columnIndex > 0) {
          ...
          onSelectionChange({ rowIndex: rowIndex - 1, columnIndex: columnKeys.length - 1 });
        }
        return true;
      ...
      case "home":
        if (isEditing) return false;
        detail.preventDefault();
        if (detail.rawEvent && (detail.rawEvent as React.KeyboardEvent).ctrlKey) {
          onSelectionChange({ rowIndex: 0, columnIndex: 0 });
        } else {
      ...
      case "end":
        ...
        detail.preventDefault();
        if (detail.rawEvent && (detail.rawEvent as React.KeyboardEvent).ctrlKey) {
          onSelectionChange({ rowIndex: rows.length - 1, columnIndex: columnKeys.length - 1 });
        } else {
      ...
      case "paste":
        if (hasSelection && onPaste && isCurrentEditable) {
          detail.preventDefault();
          const text = (detail.rawEvent as React.ClipboardEvent).clipboardData.getData("text");
          if (text) {
