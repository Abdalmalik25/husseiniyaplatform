...
class SEOAgent {
  ...
  async analyze() {
    ...
    report.score = (report.metrics.performance + report.metrics.accessibility + 
                   report.metrics.bestPractices + report.metrics.seo) / 4;
  ...
  analyzeOpenGraph() {
    ...
    try {
      ...
      const ogRequired = ['property="og:title"', 'property="og:description"', 'property="og:image"', 'property="og:url"'];
      const missingOG = [];
...
if (import.meta.url === `file://${process.argv[1]}`) {
  agent.analyze().then(async (report) => {
    ...
    try {
      writeFileSync(join(resultsDir, 'seo-report.json'), JSON.stringify(report, null, 2));
    } catch {}
  }).catch(console.error);
}
