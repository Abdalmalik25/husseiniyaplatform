...
class MonitoringAgent {
  ...
  async analyze() {
    ...
    // Calculate score
    report.score = this.calculateMonitoringScore(report.metrics, report.integrations);
  ...
  checkSentryIntegration() {
    ...
    serverFiles.forEach(file => {
      try {
        if (readFileSync(file, 'utf-8').includes('initSentry') || readFileSync(file, 'utf-8').includes('Sentry.init')) {
          hasSentryInit = true;
        }
      } catch {}
    });
  ...
  calculateMonitoringScore(metrics, integrations) {
    ...
    // Metrics score (40%)
    const metricsScore = ((100 - metrics.errorRate) + (100 - metrics.responseTime / 2) + (100 - metrics.memoryUsage) + metrics.uptime) / 4 * 0.4;
    score += metricsScore;
  ...
  generateRecommendations(report) {
    ...
    if (report.metrics.responseTime > 200) {
      recs.push('Optimize response times - consider caching and query optimization');
    }
...
if (import.meta.url === `file://${process.argv[1]}`) {
  agent.analyze().then(async (report) => {
    ...
    try {
      writeFileSync(join(resultsDir, 'monitoring-report.json'), JSON.stringify(report, null, 2));
    } catch {}
  }).catch(console.error);
}
