...
class BrandAgent {
  ...
  async analyze() {
    ...
    const totalScore = Object.values(report.brandElements).reduce((a, b) => a + b, 0);
    report.score = Math.round(totalScore / Object.keys(report.brandElements).length);
    report.issues = [...logoAnalysis.issues, ...colorAnalysis.issues, 
                     ...typographyAnalysis.issues, ...componentAnalysis.issues,
  ...
  analyzeColorPalette() {
    ...
    cssFiles.forEach(file => {
      try {
        ...
        if (content.includes('--primary') || content.includes('#0A1F44') || content.includes('#4A90E2')) {
          hasPrimaryColor = true;
        }
        if (content.includes('--secondary') || content.includes('#6C757D') || content.includes('#95A5A6')) {
  ...
  analyzeTypography() {
    ...
    cssFiles.forEach(file => {
      try {
        const content = readFileSync(file, 'utf-8');
        if (content.includes('font-family') || content.includes('IBM Plex Sans')) {
          hasFontFamily = true;
        }
  ...
  analyzeUIComponents() {
    ...
    componentFiles.forEach(file => {
      try {
        const content = readFileSync(file, 'utf-8');
        if (content.includes('button') && (content.includes('bg-') || content.includes('btn-'))) {
          hasButtonStyle = true;
        }
        if (content.includes('card') && (content.includes('border') || content.includes('shadow'))) {
          hasCardStyle = true;
        }
      } catch {}
    });
...
if (import.meta.url === `file://${process.argv[1]}`) {
  agent.analyze().then(async (report) => {
    ...
    try {
      writeFileSync(join(resultsDir, 'brand-report.json'), JSON.stringify(report, null, 2));
    } catch {}
  }).catch(console.error);
}
