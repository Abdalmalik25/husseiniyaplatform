...
async function main() {
  const rows = await sql`select id, username, "passwordHash", role, "tenantId" from users where username = 'e2e_runner'`;
  ...
  if (rows[0]) {
    ...
    const hash = await hashPassword(process.env.E2E_PASSWORD || "E2e#Runner#2026!Secure");
    const result = await sql`insert into users ("openId", "tenantId", name, email, username, "passwordHash", role) values ('e2e_test', 4, 'E2E Runner', 'e2e@husseiniya.local', 'e2e_runner', ${hash}, 'admin') returning id`;
